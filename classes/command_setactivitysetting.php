...
class block_workflow_command_setactivitysetting extends block_workflow_command {
    ...
    /***
     * Executes the command to set an activity setting based on the provided arguments and state.
     *
     * @param array $args The arguments to be parsed and executed.
     * @param mixed $state The current state of the workflow.
     * @return void
     */
    public function execute($args, $state) {
        ...
        if ($supportsgrade) {
            $instance = $DB->get_record($modulename, ['id' => $data->cm->instance], '*', MUST_EXIST);
